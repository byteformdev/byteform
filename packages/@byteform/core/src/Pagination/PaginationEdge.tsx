...
export const PaginationEdge = forwardRef<
    HTMLButtonElement,
    PaginationEdgeProps
>(
    (
        ...
        console.log(disabledProp, contextDisabled, type, page, total);
        const isDisabled =
            disabledProp ||
            contextDisabled ||
            (type === "first" ? page === 1 : page === total);

        const handleClick = () => {
            if (!isDisabled && onClick) {
                onClick();
            } else if (!isDisabled) {
                const targetPage = type === "first" ? 1 : total;
                onChange(targetPage);
            }
        };
        ...
        const defaultIcon =
            type === "first" ? IconChevronsLeft : IconChevronsRight;
...
PaginationEdge.displayName = "@byteform/core/Pagination.Edge";
